...
@WebServlet(name = "adminCadastroMedico", urlPatterns = {"/admin-cadastro-medico"})
public class AdminCadastroMedicoController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		try {
			...
			u.setCrmUsuario(request.getParameter("crm_usuario"));
			...
			Perfil p = new Perfil();
			p.setTipoPerfil(com.example.main.enums.TipoPerfil.MEDICO);
			
			UsuarioDAO.postUsuario(u, p);
			
			response.sendRedirect("admin-usuarios?sucesso=medico");
		} catch (Exception e) {
			e.printStackTrace();
			response.sendRedirect("admin-cadastro-medico?erro=falha");
		}
	}
}